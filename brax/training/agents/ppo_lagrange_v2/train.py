...
def train(
  ...
  def training_step(
    ...
    # Update Lagrange multiplier once per training step based on average cost
    # Average the mean_cost across all updates
  ...
  if restore_params is not None:
    ...
    lambda_lagr_value = restore_params[3] if len(restore_params) > 3 else jnp.array([initial_lambda_lagr], dtype=jnp.float32)
    training_state = training_state.replace(
