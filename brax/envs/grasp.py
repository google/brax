...
class Grasp(env.Env):
  ...
  def __init__(self, **kwargs):
    ...
    self._range_act = jnp.hstack([self._range_act, jnp.array([20, 20, 10])])
  ...
  def step(self, state: env.State, action: jnp.ndarray) -> env.State:
    ...
    norm = jnp.linalg.norm(target_pos - palm_pos)
    ...
    object_dist = jnp.linalg.norm(object_rel)
    planar_object_dist = jnp.linalg.norm(object_rel * jnp.array([1., 1., 0.]))
    object_dir = object_rel / (1e-6 + object_dist.reshape(-1))

    moving_to_object = .1 * self.sys.config.dt * jnp.dot(hand_vel, object_dir)
    ...
    moving_to_target = 1.5 * self.sys.config.dt * jnp.dot(
  ...
  def _get_obs(self, qp: brax.QP, info: brax.Info) -> jnp.ndarray:
    ...
    hand_to_obj_mag = jnp.linalg.norm(hand_to_obj)
    hand_to_obj_dir = hand_to_obj / (1e-6 + hand_to_obj_mag)
    hand_vel = qp.vel[self.hand_idx]
    heading_to_obj = jnp.dot(hand_to_obj_dir, hand_vel).reshape(-1)

    target_local = pos_local[self.target_idx]
    target_local_mag = jnp.linalg.norm(target_local).reshape(-1)
    target_local_dir = target_local / (1e-6 + target_local_mag)

    # object to target
    obj_to_target = qp.pos[self.target_idx] - qp.pos[self.object_idx]
    obj_to_target_mag = jnp.linalg.norm(obj_to_target).reshape(-1)
    obj_to_target_dir = obj_to_target / (1e-6 + obj_to_target_mag)

    object_vel = qp.vel[self.object_idx]
    obj_heading_to_target = jnp.dot(obj_to_target_dir, object_vel).reshape(-1)
  ...
  def _random_target(self, rng: jnp.ndarray) -> Tuple[jnp.ndarray, jnp.ndarray]:
    ...
    target_z = self.target_height * jax.random.uniform(rng3)
